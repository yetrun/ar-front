...
const { defineTransformer } = require('./transform')
...
function defineActions (Model, { actions, methods, staticMethods }) {
  // 定义 actions
  for (const [name, fn] of Object.entries(actions)) {
    ...
    Model.prototype[name] = function (...params) {
...
function defineOtherMethods (Model) {
  // 定义 clone 方法
  Object.defineProperty(Model.prototype, 'clone', {
    value: function () {
...
function prepareOptions(options) {
  ...
  const parts = ['attributes', 'computed', 'actions', 'methods', 'static']
...
class ModelFactory {
  ...
  extend (options = {}) {
    ...
    // 定义 actions 和 methods
    defineActions(modelClass, { actions, methods, staticMethods })

    // 定义 Transformer 相关的选项
    defineTransformer(modelClass, { onTransform })

    // 定义 model 能够用上的其他方法
    defineOtherMethods(modelClass)
